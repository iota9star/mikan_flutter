...
enum ParticleLayer { background, middle, foreground }
...
class BubbleBackgroundState extends State<BubbleBackground> with SingleTickerProviderStateMixin {
  ...
  void createParticles(Size size) {
    ...
    // Sort particles by layer once after creation
    particles.sort((a, b) => _layerOrder(a.layer).compareTo(_layerOrder(b.layer)));
  }
  ...
  List<_Point> _poissonDiscSampling({
    ...
    final grid = List<List<_Point?>>.generate(gridHeight, (_) => List<_Point?>.filled(gridWidth, null));
  ...
  _LayerSettings _getLayerSettings(ParticleLayer layer) {
    switch (layer) {
      ...
      case ParticleLayer.middle:
        return _LayerSettings(radiusRange: (random) => random.nextDouble() * 50 + 50, speedMultiplier: 0.6, alpha: 0.3);
      case ParticleLayer.foreground:
        return _LayerSettings(radiusRange: (random) => random.nextDouble() * 40 + 30, speedMultiplier: 1.0, alpha: 0.5);
    }
  }
...
class _LayerSettings {
  _LayerSettings({required this.radiusRange, required this.speedMultiplier, required this.alpha});
