...
import 'package:mikan/features/home/presentation/widgets/index.dart' show showSettingsPanel;
...
import 'package:mikan/features/settings/presentation/widgets/card_setting.dart';
import 'package:mikan/features/settings/presentation/widgets/donate.dart';
import 'package:mikan/features/settings/presentation/widgets/select_mirror.dart';
import 'package:mikan/features/settings/presentation/widgets/select_tablet_mode.dart';
import 'package:mikan/features/settings/presentation/widgets/theme_color.dart';
    ...
    return SliverPinnedAppBar(
      ...
      actions: [
        IconButton(
          onPressed: () async {
            if (!hasLogin) {
              await Navigator.pushNamed(context, Routes.login.name);
              return;
            }

            await wrapLoading(() async {
              try {
                await MikanApi.logout();
              } catch (_) {
                // Fall back to clearing local auth state below.
              }

              await MikanApi.clearCookies();
              await MyHive.removeLogin();
