...
import 'package:mikan/core/widgets/loading_widgets.dart';
...
class _RssSlivers extends ConsumerWidget {
  const _RssSlivers({required this.controller});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return MultiSliver(
      ...
      children: [
        _RssSection(rss: rss),
        _RssList(rss: rss, controller: controller),
      ],
    );
  }
}
...
class _RssList extends ConsumerWidget {
  const _RssList({required this.rss, required this.controller});
